using System.Xml.Serialization;

namespace TDS_Server.Dto.ClothesMeta
{
    #nullable disable
    public class ComponentTextures
    {
        [XmlElement("pedComponentTexture0")]
        public Component Texture0 { get; set; }

        [XmlElement("pedComponentTexture1")]
        public Component Texture1 { get; set; }

        [XmlElement("pedComponentTexture2")]
        public Component Texture2 { get; set; }

        [XmlElement("pedComponentTexture3")]
        public Component Texture3 { get; set; }

        [XmlElement("pedComponentTexture4")]
        public Component Texture4 { get; set; }

        [XmlElement("pedComponentTexture5")]
        public Component Texture5 { get; set; }

        [XmlElement("pedComponentTexture6")]
        public Component Texture6 { get; set; }

        [XmlElement("pedComponentTexture7")]
        public Component Texture7 { get; set; }

        [XmlElement("pedComponentTexture8")]
        public Component Texture8 { get; set; }

        [XmlElement("pedComponentTexture9")]
        public Component Texture9 { get; set; }

        [XmlElement("pedComponentTexture10")]
        public Component Texture10 { get; set; }

        [XmlElement("pedComponentTexture11")]
        public Component Texture11 { get; set; }

    }
    #nullable restore
}
