...
namespace TDS_Server.Handler.Entities.GTA.GTAPlayer
{
    partial class TDSPlayer
    {
        ...
        public override void AddToChallenge(ChallengeType type, int amount = 1, bool setTheValue = false)
        {
            try
            {
                ...
                for (int i = list.Count - 1; i >= 0; --i)
                {
                    ...
                    NAPI.Task.Run(() => _challengesHandler.SyncCurrentAmount(this, challenge));

                    if (challenge.Frequency == ChallengeFrequency.Forever)
                    {
                        list.RemoveAt(i);
                        Database.ExecuteForDBAsyncWithoutWait(async dbContext =>
                        {
        ...
        public override void InitChallengesDict()
        {
