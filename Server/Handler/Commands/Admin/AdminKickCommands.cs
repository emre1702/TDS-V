...
namespace TDS.Server.Handler.Commands.Admin
{
    public class AdminKickCommands
    {
        ...
        public void KickPlayer(ITDSPlayer player, TDSCommandInfos cmdinfos, ITDSPlayer target, [RemainingText(MinLength = 4)] string reason)
        {
        ...
        public async Task LobbyKick(ITDSPlayer player, TDSCommandInfos cmdinfos, ITDSPlayer target, [RemainingText(MinLength = 4)] string reason)
        {
            if (player == target)
                return;
