...
namespace TDS_Client.Handler.Draw.Dx
{
    internal class DxProgressRectangle : DxBase
    {
        ...
        public DxProgressRectangle(string text, float x, float y, float width, float height,
            ...
            UIResText.Alignment alignmentX = UIResText.Alignment.Centered, ToServerEvent ToServerEvent = ToServerEvent.Center, bool relativePos = true, int frontPriority = 0) 
            : base(frontPriority: frontPriority)
        {
            ...
            float textY = GetTextY(y, height, ToServerEvent);

            this._text = new DxText(text, textX, textY, textScale, textColor, textFont, UIResText.Alignment.Centered, ToServerEvent.Center, relativePos, false, true, frontPriority: frontPriority + 2);
            ...
            _backRect = new DxRectangle(x, y, width, height, backColor, alignmentX, ToServerEvent, relativePos, frontPriority: frontPriority);
            _frontRect = new DxRectangle(frontRectX, frontRectY, frontRectWidth, frontRectHeight, progressColor, UIResText.Alignment.Left, ToServerEvent, false, frontPriority: frontPriority + 1);
        ...
        private float GetTextY(float y, float height, ToServerEvent alignment)
        {
            if (alignment == ToServerEvent.Center)
                return y;
            else if (alignment == ToServerEvent.Top)
                return y + height / 2;
            else
                return y - height / 2;
        }
