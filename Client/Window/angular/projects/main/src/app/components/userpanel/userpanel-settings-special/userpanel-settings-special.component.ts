...
export class UserpanelSettingsSpecialComponent implements OnInit, OnDestroy {
    ...
    saveChanges() {
        ...
        this.rageConnector.callCallback(ToClientEvent.GetHashedPassword, [confirmedPassword], (hashedPassword) => {
            const username = (this.formGroup.get('usernameControl').value as string).trim();
            if (
                (!this.hasToBuyUsername || (this.formGroup.get('usernameBuyControl').enabled && username.length)) &&
                this.userpanelService.allSettingsSpecial[0] != username
            ) {
                this.rageConnector.callCallbackServer(
                    ToServerEvent.SaveSpecialSettingsChange,
            ...
            if (password && password.length) {
                this.rageConnector.callCallback(ToClientEvent.GetHashedPassword, [password], (hashedNewPassword) => {
                    this.rageConnector.callCallbackServer(
                        ToServerEvent.SaveSpecialSettingsChange,
