...
import { BodyAppearanceData } from '../../models/body-appearance-data';
...
@Component({
    ...
    selector: 'body-appearance',
    templateUrl: './body-appearance.component.html',
    styleUrls: ['./body-appearance.component.scss'],
})
export class BodyAppearanceComponent implements OnInit, OnDestroy {
    @Input() data: BodyAppearanceData;
    @Output() dataChange = new EventEmitter<BodyAppearanceData>();
    ...
    onAppearanceDataChanged(overlayId: number, dataIndex: number, event: MatSelectChange) {
        ...
        if (dataIndex % 2 == 0) {
            this.rageConnector.call(ToClientEvent.BodyDataChanged, BodyDataKey.Appearance, overlayId, event.value, this.data[dataIndex + 1]);

            // dataIndex % 2 == 1  =>  Item opacity changed
        } else {
            this.rageConnector.call(ToClientEvent.BodyDataChanged, BodyDataKey.Appearance, overlayId, this.data[dataIndex - 1], event.value);
        }
    }
