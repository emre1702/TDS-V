...
namespace TDS_Client.Manager.Lobby
{
    internal static class MapDataManager
    {
        ...
        public static void SetMapData(ClientSyncedDataDto mapData)
        {
            ...
            if (_mapInfo == null)
                _mapInfo = new DxText(MapData.Name, 0.01f, 0.995f, 0.2f, Color.White, alignmentX: RAGE.NUI.UIResText.Alignment.Left, ToServerEvent: ToServerEvent.Bottom);
            else
        ...
        private static void LoadMap(ClientSyncedDataDto map)
        {
            ...
            if (map.Target != null)
            {
                var obj = new MapObject(Constants.TargetHash, map.Target.ToVector3(), new Vector3(), dimension: Player.LocalPlayer.Dimension);
            ...
            if (map.BombPlaces != null)
            {
                ...
                {
                    var obj = new MapObject(Constants.BombPlantPlaceHash, data.ToVector3(), new Vector3(), dimension: Player.LocalPlayer.Dimension);
                    obj.FreezePosition(true);
                    obj.SetInvincible(true);
                    _objects.Add(obj);
                }
