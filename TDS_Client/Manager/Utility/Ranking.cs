    ...
    class Ranking
    {
        ...
        private static void OnRender()
        {
            ...
            if (!(_winner is null) && _winner.Exists) 
                Nametag.DrawNametag(_winner.Handle, "1. " + _winner.GetDisplayName(), 7f);
            if (!(_second is null) && _second.Exists)
                Nametag.DrawNametag(_second.Handle, "2. " + _second.GetDisplayName(), 7f);
            if (!(_third is null) && _third.Exists)
                Nametag.DrawNametag(_third.Handle, "3. " + _third.GetDisplayName(), 7f);
