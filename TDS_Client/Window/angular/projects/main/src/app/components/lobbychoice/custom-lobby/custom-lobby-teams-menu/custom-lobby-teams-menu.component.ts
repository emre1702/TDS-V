...
export class CustomLobbyTeamsMenuComponent {
  ...
  clickedOnBlipColor(blip: BlipColor) {
    this.editingTeam.BlipColor = blip.ID;
  ...
  addTeam() {
    ...
    this.teams = [...this.teams, {
      Name: defaultData.name || "Team" + this.teams.length,
      BlipColor: defaultData.blipColor,
      Color: defaultData.color,
      SkinHash: defaultData.skinHash,
      IsReadOnly: false
    }];
    this.changeDetector.detectChanges();
  }
  ...
  isValid(): boolean {
    ...
    for (const team of this.teams) {
      if (team.Name.length < 1 || team.Name.length > 25)
        return false;
    }
    return true;
  }
