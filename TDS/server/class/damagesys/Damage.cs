...
namespace Class {
	partial class Damagesys {
		...
		private static ConcurrentDictionary<int, int> damageDictionary = new ConcurrentDictionary<int, int> {
			//[ Handguns ]//
			[453432689] = 26,		//Pistol
		...
		private static ConcurrentDictionary<int, double> headMultiplicator = new ConcurrentDictionary<int, double> {
			[100416529] = 5.0,
			[205991906] = 5.0,
			[-952879014] = 5.0
		};
		private ConcurrentDictionary<int, int> customDamageDictionary = new ConcurrentDictionary<int, int> ();
		private ConcurrentDictionary<int, double> customHeadMultiplicator = new ConcurrentDictionary<int, double> ();

		public ConcurrentDictionary<Client, ConcurrentDictionary<Client, int>> allHitters = new ConcurrentDictionary<Client, ConcurrentDictionary<Client, int>> ();
		public ConcurrentDictionary<Client, Client> lastHitterDictionary = new ConcurrentDictionary<Client, Client> ();
		public ConcurrentDictionary<Client, int> playerDamage = new ConcurrentDictionary<Client, int> ();
		...
		private void DamagePlayer ( Client player, Client hitted, int damage ) {
			...
			if ( !this.allHitters.ContainsKey ( hitted ) ) {
				this.allHitters.TryAdd ( hitted, new ConcurrentDictionary<Client, int> () );
			}
			this.allHitters[hitted].AddOrUpdate ( player, damage, ( k, v ) => v + damage );
		}
