...
namespace TDS.server.manager.userpanel {
	...
	partial class Userpanel {
		...
		public static void ClientCreateReport ( Client player, string title, string text, uint forminadminlvl ) {
			...
			ReportsLog.Creation( report.ID, report.AuthorUID, title, text );
		...
		public static void ClientAddTextToReport ( Client player, uint reportid, string text ) {
			...
			if ( report.AuthorUID == character.UID ) {
				...
				if ( report.Texts.Count > 1 ) {
					...
					ReportsLog.Answer( report.ID, character.UID, text );
				}	
			} else {
		...
		public static void ClientChangeReportState ( Client player, uint reportid, bool state ) {
			...
			Character character = player.GetChar();
			ReportsLog.Answer( report.ID, character.UID, character.UID == report.AuthorUID ? "Creator" : "Admin Lvl " + character.AdminLvl );
		}
			...
			if ( character.IsAdminLevel( neededAdminlvls["remove"] ) ) {
				...
				for ( int i = 0; i < reports.Count; ++i ) {
					if ( reports[i].ID == reportid ) {
						...
						ReportsLog.Remove( report.ID, character.UID, character.UID == report.AuthorUID ? "Creator" : "Admin Lvl " + character.AdminLvl );
						return;
					}
				}
			}
